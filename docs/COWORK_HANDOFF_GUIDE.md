...
- **🆕 方針 D（自己回復・標準）**: 失敗を検出したら、ログ分析 → 修正案を実装 → 再 push → 再発火 → 検証 のループを最大 N 回まで自走する。**ただし以下の場合は停止して報告**：
  1. 戦略的判断（Option A vs B のような方針選択）が必要なとき
  2. N 回試行しても解決しないとき（無限ループ防止）
  3. ブラウザ操作・SSH 鍵 passphrase など人間にしかできない作業が必要なとき
  4. 想定外の重大な状態（巨大な diff、別ブランチへの誤 push、master 履歴破壊など）を検知したとき

各計画書では A/B/C/D のどれかを明示する。**特段の理由が無い限り 2026-05-04 以降は D を標準とする**（L-016）。
...
- **L-016**: 失敗時の **方針 D（自己回復）を 2026-05-04 以降の標準とする**。Claude Code は失敗を検出したら自走で「ログ分析→修正→再 push→再発火→検証」のループを最大 N 回試行する。ただし戦略判断・人間限定作業・想定外の重大状態が出たら停止して報告。詳細は §8-3。**初回テスト時のみ方針 C で慎重に進めるのは可**、ただし以後は D に戻すこと。理由: Cowork が逐一指示すると、コマンド実行先（PowerShell / SSH）の取り違え事故（実例: 2026-05-04、PowerShell 用コマンドを SSH セッションに貼り付けて bash 構文エラー多発）が起きやすく、エージェントに自走させた方がむしろ安全
