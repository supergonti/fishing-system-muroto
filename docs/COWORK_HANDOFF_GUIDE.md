...
| 環境 | 意味 |
|---|---|
| **🖥️ ノートPC（PowerShell）** | Windows のスタートメニュー → PowerShell で開いた黒い画面。プロンプトが `PS C:\...> ` |
| **🖥️ ノートPC（cmd）** | Windows のスタートメニュー → コマンドプロンプトで開いた黒い画面。プロンプトが `C:\...> ` |
| **🐧 sandbox-01（SSH 接続先）** | ノートPC PowerShell から `ssh sandbox-01` で接続したミニPC。プロンプトが `supergonti@sandbox-01:~$ ` |
| **🤖 Claude Code（対話プロンプト）** | sandbox-01 上で `claude --dangerously-skip-permissions` を実行した後の Claude Code の入力欄 |
| **🌐 GitHub Web UI** | ブラウザで github.com を開いて画面操作 |
...
bash "C:/Claude/tools/scan_3layer.sh" "C:/Dev/<repo>"
# exit 0 を確認（PowerShell では $LASTEXITCODE で確認）
echo $LASTEXITCODE
